...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['set_grade'])) {
    ...
    if ($student_class_id && $grade !== '') {
        $stmt = $pdo->prepare("INSERT INTO grades (student_class_id, grade) VALUES (?, ?) ON DUPLICATE KEY UPDATE grade = VALUES(grade)");
        try {
            $stmt->execute([$student_class_id, $grade]);
            $message = "<span style='color:green'>Grade set successfully.</span>";
        } catch (Exception $e) {
        ...
        <select name="student_class_id" required>
            ...
            <?php foreach ($student_classes as $sc): ?>
                <option value="<?= htmlspecialchars($sc['id']) ?>">
                    <?= htmlspecialchars($sc['student_number']) ?> - <?= htmlspecialchars($sc['student_name']) ?> | <?= htmlspecialchars($sc['subject_code']) ?> - <?= htmlspecialchars($sc['subject_name']) ?> | <?= htmlspecialchars($sc['section_name']) ?> | <?= htmlspecialchars($sc['term']) ?> | <?= htmlspecialchars($sc['school_year']) ?>
                </option>
            <?php endforeach; ?>
        </select><br>
        <label>Grade:</label>
